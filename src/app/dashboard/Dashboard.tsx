          ...
          {loading ? (
            ...
            filtered.map(msg => (
              ...
              >
                ...
                <p style={{
                  fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: 'var(--color-subtle)', lineHeight: 1.5,
...
/* ─────────────────────────────────────────
   Projects Tab
───────────────────────────────────────── */
const emptyForm = { title: '', role: '', year: '', tags: '', desc: '', liveUrl: '', figmaUrl: '' };
...
function ProjectsTab({ projects, loading }: { projects: Project[]; loading: boolean }) {
  ...
  const handleSave = async () => {
    if (!form.title || !form.role || !form.year || !form.desc) return;
    setSaving(true);
    try {
      if (editId) {
          ...
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', gridColumn: '1 / -1' }}>
                <label style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--color-muted)', letterSpacing: '0.08em' }}>Project Title *</label>
                <input
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                <label style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--color-muted)', letterSpacing: '0.08em' }}>Role *</label>
                <input
              ...
              {/* Year */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                <label style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--color-muted)', letterSpacing: '0.08em' }}>Year *</label>
                <input
              ...
              {/* Tags */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', gridColumn: '1 / -1' }}>
                <label style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--color-muted)', letterSpacing: '0.08em' }}>Tags (comma separated)</label>
                <input
                  value={form.tags} onChange={e => setForm(f => ({ ...f, tags: e.target.value }))}
                  placeholder="e.g. Web App, UI/UX, React" style={inputStyle}
                  onFocus={e => (e.target.style.borderColor = 'var(--color-accent)')}
                  onBlur={e  => (e.target.style.borderColor = 'var(--color-border)')}
                />
              </div>
              ...
              {/* Description */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', gridColumn: '1 / -1' }}>
                <label style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--color-muted)', letterSpacing: '0.08em' }}>Description *</label>
                <textarea
              ...
              {/* Live URL */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                <label style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--color-muted)', letterSpacing: '0.08em' }}>Live URL (optional)</label>
                <input
              ...
              {/* Figma URL */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                <label style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--color-muted)', letterSpacing: '0.08em' }}>Figma URL (optional)</label>
                <input
                  value={form.figmaUrl} onChange={e => setForm(f => ({ ...f, figmaUrl: e.target.value }))}
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-1)' }}>
                  <p style={{
                    fontFamily:  'var(--font-body)',
                    fontSize:    'var(--text-sm)',
                    color:       project.visible ? 'var(--color-text)' : 'var(--color-muted)',
                    fontWeight:  400,
                    ...
                    <div style={{ display: 'flex', gap: 'var(--space-1)', flexWrap: 'wrap' }}>
                      {project.tags.split(',').slice(0, 3).map(tag => (
                        <span key={tag} style={{
                          ...
                          border: '1px solid var(--color-border)', borderRadius: '100px', padding: '1px 7px',
                        }}>{tag.trim()}</span>
                      ))}
                    </div>
                  )}
